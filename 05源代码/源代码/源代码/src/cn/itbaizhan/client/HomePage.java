...
public class HomePage extends JFrame{
	...
	JLabel jLRegist=new JLabel("??????");
	JLabel jLFindPass= new JLabel("???????");
	JTextField jTusernumber=new JTextField();//????????
	JPasswordField jPassword=new JPasswordField();//?????????
	JCheckBox rememberPass=new JCheckBox("???????");
	JCheckBox Autologon =new JCheckBox("??????",true);
	...
	public void init()
	{
		...
		jBenter.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(userName.equals(""))
				{
					JOptionPane.showMessageDialog(jBenter, "????????????????");
				...
				else
				{
					HomePage.this.setVisible(false);
						try {
						ip=InetAddress.getByName("123.192.240.248");
						port=(6544);
		...
		class JlRegist extends JLabel
		{
	     ...
	     JlRegist jLRegist=new JlRegist("??????");
			...
			JlFindPass jLfindPass=new JlFindPass("???????");
